...
for stack in masks_dir:
    ...
    for sliceNo,sliceName in enumerate(slices):
        ...
        for i,j in enumerate(np.int64(mask_centroids)):
            ...
            ret,tres_image1 = cv2.threshold(np.uint8(tres_input[:,:,2]),30,255,cv2.THRESH_BINARY | cv2.THRESH_OTSU)
            tres_contours,_=cv2.findContours(tres_image1,cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
dice_coeff_df.to_csv('./dice_coeff_otsu_tresholding.csv')

dice_coeff_df = pd.DataFrame(IOU_arr)
dice_coeff_df.to_csv('./IOU_otsu_tresholding.csv')
