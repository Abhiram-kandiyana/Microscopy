...
path=r'D:\mircroscopy-1\16bitimages\slide1-manual-masks\accepted'
...
img_folder = r'D:\mircroscopy-1\16bitimages\slide1-manual-masks\mean_shift_input'
slide=r'Neo_cx'
...
image_folder = r'Neo_cx'
image_path = os.path.join(path,'Neo_cx')
...
for tres in treshold_arr:
    ...
    for stack in image_dir:
        ...
        width=0
        height=0
        slices = os.listdir(os.path.join(img_folder,image_folder,stack))
        for sliceNo,sliceName in enumerate(slices): 
            img_path = os.path.join(img_folder,image_folder,stack,sliceName)  
            ...
            # print(os.path.join(mask_folder,image_folder,stack,sliceName))
            mask_img = cv2.imread(os.path.join(mask_folder,image_folder,stack,str(sliceNo)+'.png'))[:,:,2]
        ...
        flag2=False
        flag3=False
        annotation_path = os.path.join(mask_folder,image_folder,stack)
        # print(np.unique(labels))
        for i,j in enumerate(np.int64(mask_centroids_arr)):
            ...
            if labels[j[0],j[2],j[1]] != 100:
                ...
                print(os.path.join(annotation_path,str(j[0])+".png"))
                # cv2.imshow("annotated_img1",annotated_img1)
                # cv2.waitKey()
                mask_contours,hv2 = cv2.findContours(annotated_img1,cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                for k,l in enumerate(mask_contours):
                    if(cv2.pointPolygonTest(l,(int(j[1]),int(j[2])),True) >= 0):
                ...
                for l,y in enumerate(cl_contours):
                    if(cv2.pointPolygonTest(y,(int(j[1]),int(j[2])),True) >= 0):
                        ...
                        cv2.drawContours(emp_img1,cl_contours,l,(255,255,255),-1)
                        # cv2.imshow("emp_img1",emp_img1)
                        # cv2.waitKey()
                        ##approxPolyDp--End
                        #GaussBlur--start
                ...
                dice_coeff_stack.append(dice_coeff)
    ...
    dice_coeff_df.to_csv('./dice-coeff-meanshift.csv')

    IOU_df = pd.DataFrame(IOU_arr)
    IOU_df.to_csv('IOU-meanshift.csv')
