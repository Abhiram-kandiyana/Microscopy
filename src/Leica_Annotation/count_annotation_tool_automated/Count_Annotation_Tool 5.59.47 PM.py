...
def xy(event):
    ...
    canvas.focus_set()
    cell['sliceNo'] = i
    cell['centroid'] = (round((lastx - CANVAS_IMAGE_X_SHIFT) / SFACTOR),
                        round((lasty - CANVAS_IMAGE_Y_SHIFT) / SFACTOR))  # centroid of contour (cX,cY)
    print(cell['centroid'])
...
def key(event):
    ...
    if event.keysym == "Up":
        ...
        canvas.create_line(x - MARKER_SIZE, y + MARKER_SIZE, x + MARKER_SIZE, y - MARKER_SIZE, width=MARKER_WIDTH,
                           fill="#000000")
        canvas.create_line(x - MARKER_SIZE, y - MARKER_SIZE, x + MARKER_SIZE, y + MARKER_SIZE, width=MARKER_WIDTH,
                           fill="#000000")
        canvas.update()
...
def get_centroids(event):
    ...
    radius = 4
    x,y  = round((event.x - CANVAS_IMAGE_X_SHIFT) / SFACTOR),round((event.y - CANVAS_IMAGE_Y_SHIFT) / SFACTOR)
    centroids = ANNOTATION_DICT['cells']
    sameSliceCentroids = [d['centroid'] for d in centroids if d['sliceNo'] == i]
    for centroid in sameSliceCentroids:
        if math.sqrt(math.pow(x - centroid[0], 2) + math.pow(y - centroid[1], 2)) < centroidRemovalRadius:
            print("marking at {} to be removed".format())
...
def euclidean_distance(listOfloc, x, y, radius = 8):
    foundMatch = False
    locWithinRadius = []
    for loc in listOfloc:
        ...
        if (math.sqrt(math.pow(x - loc[0], 2) + math.pow(y - loc[1], 2))) < radius:
            foundMatch = True
            # print('Removing')
            locWithinRadius.append((loc[0],loc[1]))
    ...
    try:
        if MODE == 'new':
            ...
            print('Invalid mode. Please select "new" for new annotation and "update" to update already existing '
                  'annotation based on prediction from pre-trained model.')
            sys.exit()
        refImg = cv2.resize(refImg, None, fx=SFACTOR, fy=SFACTOR,
                            interpolation=cv2.INTER_CUBIC)  # resize by scale factor
...
def iterateFunction():
    global mouseX, mouseY, ANNOTATION_DICT, IS_VALID_STACK,NameOfStack,REF_IMG_DIR
    path2Case = dirname
    # print(dirname)
    # print(dirname)
        ...
        if (section.startswith(sectionNameStartsWith) and os.path.isdir(os.path.join(path2Case, section))):
            ...
            for stack in Stacks:
                ...
                if (stack.startswith(stackNameStartsWith) and os.path.isdir(os.path.join(path2Case, section, stack))):
                    ...
                    try:
                        with open(os.path.join(dirname, save_annotation_folder_name, "ManualAnnotation.json"),
                                  'r+') as read_fp:
                    ...
                    if MODE == 'update':
                        ...
                        annotation_case[
                            'total_count'] -= old_count  # subtract old count and add updated count if valid stack
                        try:
                            os.remove(os.path.join(path2Case, save_annotation_folder_name, NameOfStack + '.png'))
                        except:
                            print("{} not found".format(os.path.join(path2Case, save_annotation_folder_name, NameOfStack + '.png')))
                        annotation_case[
                            'valid_stacks'] -= 1  # remove from valid stack count and add later if valid stack.
...
def remove_centroid():
    ...
    funcId1 = canvas.tag_bind(imgID, "<Button-1>", get_centroids)
    # canvas.tag_bind(imgID, "<B1-Motion>", addLine)
    funcId2 = canvas.tag_bind(imgID, "<ButtonRelease-1>", save_cell)
