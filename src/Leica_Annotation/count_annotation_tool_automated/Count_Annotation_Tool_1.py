...
global StackSize
StackSize = 10
SFACTOR = 5  # scaling factor for viz
...
def get_centroids(event):
    ...
    for centroid in sameSliceCentroids:
        if math.sqrt(math.pow(cursorX - centroid[0], 2) + math.pow(cursorY - centroid[1], 2)) < centroidRemovalRadius:
            centroidsNearBy.append(centroid)
    ...
    ANNOTATION_DICT['cells'] = centroids
...
def ReadSequenceOfImages(image_folder, NameOfStack):
    ...
    for mark in ANNOTATION_DICT['cells']:
        ...
        canvas.create_line(x - 3, y + 3, x + 3, y - 3, width=2, fill="#00ff00")
        canvas.create_line(x - 3, y - 3, x + 3, y + 3, width=2, fill="#00ff00")
        canvas.update()
                ...
                if (stack.startswith(stackNameStartsWith) and os.path.isdir(os.path.join(path2Case, section, stack))):
                    ...
                    if MODE == 'update':
                        ...
                        try:
                            ...
                            print("Error when finding the file {}: {}".format(os.path.join(path2Case, save_annotation_folder_name, NameOfStack + '.png'),e))
                        annotation_case[
