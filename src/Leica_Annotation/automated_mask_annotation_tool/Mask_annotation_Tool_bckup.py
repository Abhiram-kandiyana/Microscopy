...
SCALE_FACTOR = 5  # Must be Interger - scale image by this factor for better visual
...
def ReadSequenceOfImages(image_folder, saveTo_folder, NameOfStack, ref_annotation_stack_cells,section):
    ...
    if MODE == 'new':  # initialize annotation dict and MASKS for this stack if mode is 'new'. Append to already available if mode is 'update'
        ...
        for mask_no,mask_name in enumerate(listOfImages):
            # print(os.path.join(saveTo_folder, mask_no))
            mask = cv2.imread(os.path.join(saveTo_folder, str(mask_no)+'.png'), -1)
        ...
        if (section.startswith(sectionNameStartsWith) and os.path.isdir(os.path.join(path2Case, section))):
            ...
            for stack in Stacks:
                ...
                if (stack.startswith(stackNameStartsWith) and os.path.isdir(os.path.join(path2Case, section, stack))):
                    ...
                    #         if section in annotation_case and any(ann.get('StackName', None) == NameOfStack for ann in annotation_case[section]):
                    ...
                    #                 continue
                    #         # try:
                    #         #     if stack_name in annotation_case[section]:
                    #         #         continue
                    #         #     else:
                    #         #         annotation_case[section].append({})
                    #         # except:
                    #         #     print("Error while trying to check if this stack is already annotated")
                    # except IOError:
