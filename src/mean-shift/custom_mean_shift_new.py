...
for cluster_radius in radius_arr:
    radius_dir = "radius_"+str(cluster_radius)
    for tres in seed_treshold_arr:
        for tres1 in data_treshold_arr:
            ...
            for stackNo,stackName in enumerate(masks):
                    ...
                    def fit(self, data):
                        ...
                        while True:
                            ...
                            for k in range(len(data)):
                                featureset = data[k]
                                distances = [weightedEuclideanDist(featureset,centroids[centroid],[0.2,0.4,0.4,0]) for centroid in centroids]
                ...
                if mode == mc_constants.algo_test or mc_constants.test:
                    random.seed(221)
                    labels_path = os.path.join('./labels',mode,radius_dir,treshold_dir_name, stackName)
                ...
                marked_img_dir_path1 = os.path.join(os.path.join(marked_img_path,mode,radius_dir,treshold_dir_name),stackName)
                ...
                result_dir = stackName
                # curr_res_path = os.path.join(result_path,result_dir)
                predicted_cluster_img_dir_path = os.path.join(mc_constants.predicted_clusters_images_path,mode,radius_dir,treshold_dir_name,stackName)
                black_color = [0,0,0]
                # print("clusters in 8th image",np.unique(labels[8]))
                # print("clusters in 9th image",np.unique(labels[9]))
                if not os.path.exists(predicted_cluster_img_dir_path):
            ...
            try:
                result_csv_df.to_csv(os.path.join(csv_results_path,mode,radius_dir, treshold_dir_name+'.csv'))
            except:
            ...
            print("seed_thresh",tres)
            print("data_thresh",tres1)
            print("radius",cluster_radius)
            ...
            result_json_list.append({"seed threshold":tres,"data treshold":tres1,"radius":cluster_radius, "error rate":error,"Accuracy":accuracy,"Recall":recall,"precision": precision,"f1_score":f1_score,"Total true positives":total_tp,"Total false positives":total_fp,"Total false negatives": total_fn,"Total Groundtruth":total_gt})
            try:
            ...
            if(recall>max_recall):
                min_error=error
                best_seed_treshold=tres
                best_data_treshold = tres1
                best_radius = cluster_radius
...
print("---------------------------------")
print("Best seed Treshold ",best_seed_treshold)
